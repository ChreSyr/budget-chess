...
///
/// A PGN game is composed of [PgnHeaders] and moves represented by a [PgnNode]
/// tree.
///
/// ## Parser
///
...
///
/// You can use [PgnNode.transform] to augment all nodes in the game tree with
/// user data.
///
/// It allows you to provide context. You update the context inside the
/// callback. Context object itself should be immutable to prevent any unwanted
/// mutation.
...
class PgnGame<T extends PgnNodeData> {
  ...
  static PgnGame<PgnNodeData> parsePgn(
    ...
    final games = <PgnGame<PgnNodeData>>[];
    _PgnParser(
      games.add,
      initHeaders,
    ).parse(pgn);
  ...
  String makePgn() {
    ...
    final stack = <_PgnFrame>[];

    if (moves.children.isNotEmpty) {
      final variations = moves.children.iterator..moveNext();
      stack.add(
        _PgnFrame(
          state: _PgnState.pre,
          ply: initialPly,
        ...
        case _PgnState.sidelines:
          {
            ...
            if (child) {
              ...
              if (frame.node.children.isNotEmpty) {
                final variations = frame.node.children.iterator..moveNext();
                stack.add(
                  _PgnFrame(
                    state: _PgnState.pre,
                    ply: frame.ply + 1,
